...
async def read_contact_birthdays(
    ...
    **{from_date} + 7 days** will be returned.
    ...
    if from_date and to_date:
        if from_date > to_date:
            raise HTTPException(
                ...
                detail="Cannot be greater than {to_date} parameter"
            )
    ...
    return await repository_contacts.get_contacts_birthdays(from_date, to_date, db)
...
async def partial_contact_update(body: ContactPartialUpdateModel, contact_id: int, db: Session = Depends(get_db)):
    contact = await repository_contacts.partial_update_contact(contact_id, body, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found")
    return contact
